...
SIGNER="{{ gpg_key_name }}"
VERSION={{ defcoin_version }}
commit=false
url={{ defcoin_git_repo_url }}
proc=2
mem=3584
lxc=true
...
defcoin_repo_dir_path=${HOME}/defcoin
gitian_descriptor_path=${defcoin_repo_dir_path}/contrib/gitian-descriptors/gitian-linux.yml

defcoin_binaries_dir_path=${HOME}/defcoin-binaries
...
Run this script from the directory containing the defcoin, gitian-builder, and gitian.sigs repositories.
...
-u|--url	Specify the URL of the repository. Default is {{ defcoin_git_repo_url }}
...
then
	# Make output folder
	mkdir -p ${defcoin_binaries_dir_path}/${VERSION}
	...
	then
        for suite in ${suites} ; do
            ...
            make -C ${defcoin_repo_dir_path}/depends download SOURCES_PATH=${gitian_builder_repo_path}/cache/common
            ...
            ./bin/gbuild -j ${proc} -m ${mem} --commit defcoin=${COMMIT} --url defcoin=${url} ${suite_dir_path}/gitian-linux.yml
            ...
            suite_binaries_dir_path=${defcoin_binaries_dir_path}/${VERSION}/${suite}
            mkdir ${suite_binaries_dir_path}

            mv ${build_dir_path}/out/defcoin-*.tar.gz ${build_dir_path}/out/src/defcoin-*.tar.gz ${suite_binaries_dir_path}
