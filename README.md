Defcoin deterministic builds
==========================

This is a deterministic build environment for [Defcoin](https://github.com/charlesrocket/defcoin/) that uses [Gitian](https://gitian.org/).

Gitian provides a way to be reasonably certain that the Defcoin executables are really built from the exact source on GitHub and have not been tampered with. It also makes sure that the same, tested dependencies are used and statically built into the executable.
...
```
git clone git@github.com:harrypotter/defcoin-gitian.git
```
...
```
defcoin-gitian$ cp .env.example .env
defcoin-gitian$ cp .envrc.example .envrc
...
```
defcoin-gitian$ echo $DEFCOIN_GIT_REPO_URL

direnv: error .envrc is blocked. Run `direnv allow` to approve its content.
defcoin-gitian$ direnv allow
direnv: loading .envrc
direnv: export +GIT_EMAIL +GIT_NAME +GPG_KEY_ID +GPG_KEY_NAME +DEFCOIN_GIT_REPO_URL +DEFCOIN_VERSION
defcoin-gitian$ echo $DEFCOIN_GIT_REPO_URL
https://github.com/charlesrocket/defcoin
defcoin-gitian$
```
...
```
defcoin-gitian$ type python3
python3 is /usr/local/bin/python3
defcoin-gitian$ /usr/local/python3 --version
Python 3.7.4
```
...
```
defcoin-gitian$ /usr/local/Cellar/python/3.7.4/bin/python3 -m venv local/python_v3.7.4_venv
```
...
```
defcoin-gitian$ ls -F local/python_v3.7.4_venv/
...
```
defcoin-gitian$ ls -F local/python_v3.7.4_venv/bin/
...
```
defcoin-gitian$ echo "load_prefix local/python_v3.7.4_venv" >> .envrc
direnv: error .envrc is blocked. Run `direnv allow` to approve its content.
defcoin-gitian$ direnv allow
direnv: loading .envrc
direnv: export +CPATH +GIT_EMAIL +GIT_NAME +GPG_KEY_ID +GPG_KEY_NAME +LD_LIBRARY_PATH +LIBRARY_PATH +MANPATH +PKG_CONFIG_PATH +DEFCOIN_GIT_REPO_URL +DEFCOIN_VERSION ~PATH
defcoin-gitian$
```
...
```
defcoin-gitian$ echo $PATH
/Users/harrypotter/Projects/defcoin-gitian/local/python_v3.7.4_venv/bin:/usr/local/bin:/usr/bin:/bin:/usr/local/games:/usr/games
defcoin-gitian$ type python
python is /Users/harrypotter/Projects/defcoin-gitian/local/python_v3.7.4_venv/bin/python
defcoin-gitian$ type python3
python3 is /Users/harrypotter/Projects/defcoin-gitian/local/python_v3.7.4_venv/bin/python3
```
...
```
defcoin-gitian % pip3 install --upgrade pip
Collecting pip
[...]
...
```
defcoin-gitian$ pip3 install --requirement requirements-pip.lock
```

Check that you can run `ansible` from the command line:

```
defcoin-gitian % ansible --version
ansible 2.9.11
[...]
defcoin-gitian %
```
...
You can generate a keypair specifically for defcoin gitian builds with a command like the one below.


```
defcoin-gitian$ gpg --quick-generate-key --batch --passphrase '' "Harry Potter (defcoin gitian) <hpotter@hogwarts.wiz>"
...
```
defcoin-gitian$ vagrant plugin install --local
```



## Configure the version of defcoin you want to build and sign

Set the value of the `DEFCOIN_VERSION` variable in `.env` to point to the defcoin commit you want to
create a signature for. Likely you want the name of a
[git-tagged defcoin version](https://github.com/charlesrocket/defcoin/tags), usually the most recent released
version.  
...
```
defcoin-gitian$ vagrant up defcoin-build
```
...
programs in the defcoin-build VM to connect to ssh-agent to perform operations with the private key.
...
```
defcoin-gitian$ ssh-add -l
The agent has no identities.

defcoin-gitian$ ssh-add ~/.ssh/github_id_rsa
...
```
defcoin-gitian$ vagrant status
Current machine states:

defcoin-build               running (virtualbox)
...
```
defcoin-gitian$ vagrant ssh defcoin-build
[...]

# on the virtualbox vm
vagrant@defcoin-build:~$ ssh-add -l
4096 d1:43:75:a7:95:65:9e:d4:8e:57:d8:98:58:7d:92:4c /home/hpotter/.ssh/github_id_rsa (RSA)

vagrant@defcoin-build:~$ ssh -T git@github.com
...
```
# on the virtualbox vm
vagrant@defcoin-build:~$ ./gitian-build.sh
```
...
Signatures can be verified by running `gitian-build.sh --verify`, but set `build=false` in the script to skip building. Run a `git pull` beforehand on `gitian.sigs` so you have the latest. The provisioning includes a task which imports Defcoin developer public keys to the Vagrant user's keyring and sets them to ultimately trusted, but they can also be found at `contrib/gitian-downloader` within the Defcoin source repository.
...
Fork the [defcoin/gitian.sigs](https://github.com/defcoin/gitian.sigs) repository by following the link
and clicking "fork".

Now you can cd into the gitian.sigs directory, set the repository to point to your fork of
[defcoin/gitian.sigs](https://github.com/defcoin/gitian.sigs), push
...
We provide two options for automatically importing keys into the VM, or you may choose to copy them manually. GPG keys are needed to sign the manifests which get pushed to [gitian.sigs](https://github.com/defcoin/gitian.sigs).
...
Uncomment the line beginning with `gitian.vm.synced_folder "~/.gnupg"` in `Vagrantfile`. Ensure the destination mount point is empty. Then run:
    ...
    vagrant sshfs --mount defcoin-build
